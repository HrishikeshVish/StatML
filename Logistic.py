...
class Logistic():
    ...
    def feature_extraction(self, raw_data, use_language_vocab=False):
        ...
        new_text = text.apply(tokenizer.tokenize)
    ...
    def stochastic_gradient_descent(self, data, labels):
        ...
        """
        #print("INSIDE STOCHASTIC GRAD")
        print(loss)
        #loss_diff = np.matmul(loss, data)/len(loss)
        ...
        #print(loss_diff*self.learning_rate)
        #print((loss_diff*self.learning_rate).shape)
        #print(new_weights)
    ...
    def train(self, labeled_data, learning_rate=None, max_epochs=None):
        ...
        however you want.
        """
        if(learning_rate != None and max_epochs !=None):
            ...
            for i in range(max_epochs):
                ...
                for j in range(0, len(train_X), batch_size):
                    ...
                    loss = [model_result - expected_result]
                    #print("HERE")
                    self.weights = self.weights - loss[0] * self.learning_rate * rows
                ...
                for j in max_iter_values:
                    ...
                    for k in range(5):
                        ...
                        for l in range(len(predictions)):
                            if((predictions[l]>=0.5 and validation_y[l] == 1) or (predictions[l]<0.5 and validation_y[l] == 0)):
                                count+=1
